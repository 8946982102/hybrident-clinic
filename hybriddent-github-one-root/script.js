...
function initActiveNav(){
  ...
  const current=body?.dataset.page || (location.pathname.split('/').pop().replace(/\.html$/,'') || 'index');
  const page=current==='index' ? 'home' : current;
  document.querySelectorAll('.nav-link').forEach(link=>{
    const href=(link.getAttribute('href')||'').split('#')[0].split('/').pop();
    const target=href==='index.html'||href==='' ? 'home' : href.replace(/\.html$/,'');
...
function initImageFallbacks(){
  const fallback='dental-fallback.svg';
  const mark=(img)=>{
...
document.addEventListener('DOMContentLoaded',()=>{initActiveNav();initImageFallbacks();initReveal();initMobileMenu();initScroll();initFaq();initGallery();initTestimonials();initBooking();initNewsletter();initChat();});
